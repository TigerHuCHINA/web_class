...
public class UserCountDao extends BaseDao {
	public int doCount(User u,User u0) {
		String sql="update user set view=view+1 where idname=? and time_to_sec(now())-(select max(time_to_sec(daytime)) as d1 from userrecord where visiting=? and visited=?)>300";
